...
def make_dataset(path, version, args, group, final_build):
    ...
    class_dict = {'BKL': 0, 'MEL': 1, 'NV': 2} #label dictionary
    samples1 = []
    samples2 = []
    samples3 = []
    samples4 = []
...
if __name__ == '__main__':
    ...
    if args.final == "true":
        ...
        train_samples, test_samples, total_cnt = make_dataset(path, 1, args, "No Group", final_build=args.final)
        data = MyDataset("No Group", "test", train_samples, test_samples)
    ...
    if args.final == "false":
        ...
        for i in range(len(groups)):
            ...
            if args.new_data == "true":
                ...
                train_data = MyDataset("No Group", "train", train_samples, test_samples)
                test_data = MyDataset("No Group", "test", train_samples, test_samples)
        ...
        if args.type == "train" or args.type == "all":
            ...
            while iter < args.epochs:
                ...
                iter += 1
                model_name = '/home/schilak/savedmodels' + "swin_final_teacher_iter" + str(version) + ".pkl"
                model_dict_name = '/home/schilak/savedmodels' + "swin_final_state_dict_teacher_iter" + str(version) + ".pkl"
        ...
        if args.type == "test" or args.type == "all":
            ...
            if args.device == "cuda":
                ...
                model = torch.load('. ed_model/' + "mobilenet_final" + str(version) + ".pkl", map_location=torch.device('cpu'))
            # model = torch.load('./saved_model/' + "swin_pretrained2_" + str(version) + ".pkl")
            model.to(args.device)
